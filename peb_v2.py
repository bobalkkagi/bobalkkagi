...
def SetPEB(uc):
    ...
    peb.ActivationContextData = 0x460000 # 언팩시 필요 일단 스택 사이즈를 줘봄
...
def SetLdrTable(uc, base, dllBase, Path, dll, dllNameAddress):
    ...
    lenDll = len(dll)*2
    try:
        uc.mem_write(dllNameAddress, Path.encode('utf-16')+b'\x00\x00')
        GLOBALVAR['dllNameSpace'] += lenPath + 2
    except:
        uc.mem_map(dllNameAddress, 0x10000)
        uc.mem_write(dllNameAddress, Path.encode('utf-16')+b'\x00\x00')
        GLOBALVAR['dllNameSpace'] += lenPath + 2
    lt.FullDllName.Length = lenPath 
    lt.FullDllName.MaximumLength = lenDll+2
    lt.FullDllName.Buffer = dllNameAddress
    lt.BaseDllName.Length = lenDll
    lt.BaseDllName.MaximumLength = lenDll+2
    lt.BaseDllName.Buffer = dllNameAddress + lenPath - lenDll
    uc.mem_write(changeBase, bytes(lt))
    ChagnePrevNodeFLink(uc, base)
    return changeBase

def ChagnePrevNodeFLink(uc, addr):
    data = struct.pack('<Q',  GLOBALVAR['LdrDataEntryTable'])
